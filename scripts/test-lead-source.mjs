...
function visit(run, storage, href, referrer, hasForm, blocked, confirmation){
  ...
  const document = {
    ...
    querySelectorAll: function(selector){
      if(selector === 'a[href*="contact.html"]') return [{addEventListener:function(event, fn){ estimateHandlers[event] = fn; }}];
      return [];
    }
  };
  run(document, window);
  return {fields:fields, handlers:handlers, estimateHandlers:estimateHandlers, window:window};
}
function suite(run){
  const storage = {};
  const service = visit(run, storage, 'https://chivvflooring.com/services/refinishing.html?utm_source=instagram&utm_medium=social&utm_campaign=floor_refresh&email=private', 'https://instagram.com/', false);
  service.estimateHandlers.click();
  ...
  check(contact.fields.page_context.value === '/services/refinishing.html', 'Estimate CTA source page must reach the form');
  ...
  check(contact.window.dataLayer[0].utm_campaign === 'floor_refresh', 'Attempt event must retain campaign attribution');
  ...
  check(confirmation.window.dataLayer[0].page_context === '/services/refinishing.html', 'Confirmed lead must retain source context');
  check(confirmation.window.dataLayer[0].utm_source === 'instagram', 'Confirmed lead must retain campaign attribution');
